...
決断①が終わると、参加者は実験群70%・統制群30%でランダムに振り分けられます。パイロットテストで両方の分岐を確実に確認したいときは、URLの末尾に `?group=experimental` または `?group=control` を付けてアクセスしてください（例: `https://.../index.html?group=control`）。

- ランダム抽選をせず、指定した群に固定されます。
- 参加者マスタには通常の振り分けと同じ形式で群が記録されますが、`groupForced=true` の印が別途付くので、本番のランダムデータと混同せず区別できます。
- 本番用に配布するリンクには付けないでください（`?group=` が無ければ常に70:30のランダム振り分けです）。
